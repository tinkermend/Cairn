...
日期：2026-09-13。状态：**已实施；2026-09-14 复查的六项缺陷已修复，定向回归通过**。
...
## 12. 复查修复约定（2026-09-14）

用户已授权修复[实施复查](../reviews/2026-09-14-platform-configuration-center-review.md)中的六项问题，范围保持四组平台配置。

- 表单值与编辑基准修订一起维护；未修改的表单跟随最新值，发生冲突时保留输入并停止旧表单保存，用户可明确放弃未保存修改后载入最新配置。保存、恢复成功同步更新表单与修订。
- 登记模型密钥时同时提交模型地址，在平台域保存不可变的 Secret → 服务 origin 绑定；保存、恢复及连接测试共用绑定校验。停用不解除绑定，带引用的停用配置也必须匹配地址，不能借停用把无绑定 Secret 写入修订后冒充存量绑定。既有引用按首次出现的不可变配置修订解释绑定，不根据当前可编辑值重新绑定；未找到可信绑定时要求重新登记。密钥明文格式、SecretProvider 与旧 Run 引用保持兼容。
- 增量迁移使用 PG `0026`、MySQL / SQLite `0010`（录制回填并行占用 `0025` / `0009`），保留既有迁移校验和。绑定表只存引用及地址，无明文密钥。
- AI 超时按最终 Run / Step 有效策略校验；旧摘要兼容逐个比较嵌套覆盖字段；Trace 天数按最终模式解析，再应用显式覆盖。
- 前端复用现有 Tabs、Form、Button 与警告状态；主操作仍是保存。验证后台刷新、409、恢复后再保存、密钥登记/测试及窄屏，不扩展全站改版。

修复已落地，验证结果与限制见[复查报告末节](../reviews/2026-09-14-platform-configuration-center-review.md#修复与回归2026-09-14)。
