...
export const CrxRecorder: React.FC = ({
  ...
  const showPreferences = React.useCallback(() => {
    const modal = createModal(({ isOpen, onResolve }) =>
      <Dialog title='偏好设置' isOpen={isOpen} onClose={onResolve}>
        <PreferencesForm />
      </Dialog>
    );
    modal().catch(() => {});
  ...
  const saveCode = React.useCallback(() => {
    ...
    const modal = createModal(({ isOpen, onResolve, onReject }) => {
      return <Dialog title='保存代码' isOpen={isOpen} onClose={onReject}>
  ...
  return <>
    ...
    <div className='recorder'>
      {settings.experimental && <>
        <Toolbar>
          <ToolbarButton icon='save' title='保存' disabled={false} onClick={saveCode}>保存</ToolbarButton>
          <div style={{ flex: 'auto' }}></div>
          <div className='dropdown'>
            <ToolbarButton icon='tools' title='工具' disabled={false} onClick={() => {}}></ToolbarButton>
            <div className='dropdown-content right-align'>
              <a href='#' onClick={requestStorageState}>下载 storage state</a>
            </div>
          </div>
          <ToolbarSeparator />
          <ToolbarButton icon='settings-gear' title='偏好设置' onClick={showPreferences}></ToolbarButton>
        </Toolbar>
      </>}
