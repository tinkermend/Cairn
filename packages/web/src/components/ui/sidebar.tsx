...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className='sr-only'>展开或收起侧栏</span>
    </Button>
  )
}
...
function SidebarInset({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'relative flex w-full flex-1 flex-col bg-surface-nav',
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar='input'
      className={cn('h-8 w-full bg-surface-control shadow-none', className)}
      {...props}
    />
  )
}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  return (
    <ul
      ...
      className={cn(
        'ms-7 flex min-w-0 flex-col gap-1 border-s border-sidebar-border px-2.5 py-0.5',
        'group-data-[collapsible=icon]:hidden',
        className
      )}
      {...props}
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      aria-current={isActive ? 'page' : undefined}
      className={cn(
        'flex h-9 min-w-0 items-center gap-2 overflow-hidden rounded-md border-s-2 border-transparent px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-[18px] [&>svg]:shrink-0 [&>svg]:text-inherit',
        'data-[active=true]:border-sidebar-primary data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground',
        size === 'sm' && 'text-xs',
        size === 'md' && 'text-sm',
        'group-data-[collapsible=icon]:hidden',
        className
