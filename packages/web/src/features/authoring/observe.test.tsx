...
vi.mock('@/features/runs/use-run-observation', () => ({
  useRunObservation: () => ({
    run: {
      ...
      debugOverlay: null,
    } as Pick<RunDetailDto, 'id' | 'status' | 'debugMode' | 'checkpoint' | 'debugOverlay'>,
    refresh,
  }),
}))
...
const found: TargetObservation = {
  ...
  diagnostics: { outcome: 'FOUND', candidatesTried: [{ index: 0, by: 'testId', value: 'go', matches: 1 }] },
  source: 'managed',
}
  ...
  it('指认只产生观察；本次验证才写覆盖；写回草稿才改本地目标', async () => {
    ...
    const screen = await render(
      <QueryClientProvider client={client}>
        <AuthoringObserveProvider runId={RUN_ID} selectedStepId={STEP_ID} enabled onApplyTarget={onApplyTarget} onWriteBack={onWriteBack}>
          <Probe />
        </AuthoringObserveProvider>
      </QueryClientProvider>,
    )
    await screen.getByRole('button', { name: '点选' }).click()
    await vi.waitFor(() => expect(observeRun).toHaveBeenCalledWith(RUN_ID, { op: 'pick', x: 12, y: 34 }))
    ...
    await vi.waitFor(() => expect(onApplyTarget).toHaveBeenCalledWith(found.target))
