 ...
 */
const SECRET_HINT =
  /((?:password|token|secret|api[_-]?key|口令|密码)["']?\s*[:=：]\s*)(?:"[^"]*"|'[^']*'|[^\s,;，；]+)/gi
const INJECTION_HINT =
  /ignore (all )?(previous|above) instructions|you are now|忘记(以上|之前)指令|系统提示/i

/** 编写期自然语言脱敏：口令、Bearer、注入提示。 */
export function redactAuthoringExpression(expression: string): string {
  return expression
