/**
 * 纯 TypeScript 实现的同步 SHA-256 哈希函数。
 * 无需依赖 Node.js 内置模块（node:crypto），能在浏览器和任意 JS 运行时中同步执行。
 */
...
export function syncSha256Bytes(bytes: Uint8Array): string {
  ...
  // 计算需要的块数：每个块 64 字节，需要追加 1 字节 0x80，8 字节长度
  ...
  const hexParts = [h0, h1, h2, h3, h4, h5, h6, h7]
  return hexParts.map((h) => h.toString(16).padStart(8, '0')).join('')
}
