-- 0082：调用方 Webhook 配置、异步投递事实与可恢复 claim。

CREATE TABLE "__SCHEMA__".service_webhooks (
  id UUID PRIMARY KEY,
...
CREATE UNIQUE INDEX service_webhooks_caller_idx
  ON "__SCHEMA__".service_webhooks (caller_id);

CREATE TABLE "__SCHEMA__".service_webhook_deliveries (
  id UUID PRIMARY KEY,
...
CREATE UNIQUE INDEX service_webhook_deliveries_event_idx
